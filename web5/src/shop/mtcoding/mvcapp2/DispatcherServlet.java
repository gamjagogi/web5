...
public class DispatcherServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 4. 컨트롤러 객체 생성(싱글톤)
        BoardController boardCon = new BoardController(Factory.getBoardRepositroy());

        // 4.5 유저 컨트롤러 객체
        UserController userCon = new UserController(Factory.getUserRepositroy());
            ...
            switch (action){
                case "login":
                    try {
                        ...
                        String loginView = boardCon.login(req,userId, userPw);
                ...
                case "admin": //회원가입 form줘!
                    String adminView = userCon.admin(req);
                ...
                case "join":
                    ...
                    String password2 = req.getParameter("userPw2");
                    ...
                    String checkPin = req.getParameter("checkPin");
                    String[] para = {userid,password,password2,username,birth,gender,email,local,phoneNumber,checkPin};
                    for(String a:para){
                        if(a==null){
                            req.setAttribute("error","빈 칸이 존재합니다.");
                            req.getRequestDispatcher("/board/admin.do").forward(req,resp);
                        }
                    }
                    String joinModel = userCon.join(req,userid,password,password2,username,birth,gender,email,local,phoneNumber,checkPin);
                    req.getRequestDispatcher(joinModel).forward(req,resp);
                    resp.sendRedirect(joinModel);
                    break;
                case "mainPage":
